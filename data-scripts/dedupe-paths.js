...
const filterByGreaterDateOrGreaterIndexPosition = (pathA) => (pathB, index) => {
...
fs.readFile(inPath, "utf8", function(err, src) {
	...
	paths.forEach(([pathAPoints, pathA], index) => {
		const memoizedFilter = filterByGreaterDateOrGreaterIndexPosition(pathA);
	...
	fs.writeFile(outPath, data, (err) => console.log(err || "done!"));
});
